...
from workload_simulator.schema.schema import TimePrivacyUnit, rdp_group_privacy
...
# # load from a file
# mechanism_cache_dir = "mechanism_cache"
# if not os.path.exists(mechanism_cache_dir):
#     os.mkdir(mechanism_cache_dir)
...
class MechanismCost:
    ...
    def set_cost_adp(self, privacy_unit: TimePrivacyUnit, epsilon, delta):
        if self.adp is None:
            self.adp = {}
        self.adp[privacy_unit.name] = {"EpsDeltaDp": {"eps": epsilon, "delta": delta}}

    def set_cost_rdp(self, privacy_unit: TimePrivacyUnit, mechanism, alphas):
        if self.rdp is None:
            self.rdp = {}
        costs = [mechanism.get_RDP(alpha) for alpha in alphas]

        self.rdp[privacy_unit.name] = {"Rdp": {"eps_values": costs}}

        # TODO: NOT SURE HOW WE'LL GET THESE REDUCED COSTS
        # copy the larger cost to smaller units
        to_privacy_units = [unit for unit in TimePrivacyUnit if unit.value < privacy_unit.value]
        for to_unit in to_privacy_units:
            self.rdp[to_unit.name] = self.rdp[privacy_unit.name]

        # apply group privacy for larger units
        to_privacy_units = [unit for unit in TimePrivacyUnit if unit.value > privacy_unit.value]

        for to_unit in to_privacy_units:
            costs = [rdp_group_privacy(mechanism, from_unit=privacy_unit, to_unit=to_unit, target_alpha=alpha) for alpha in alphas]
            self.rdp[to_unit.name] = {"Rdp": {"eps_values": costs}}
...
class MechanismWrapper(ABC):
    ...
    def __init__(self, privacy_unit: TimePrivacyUnit, cost_calibration: BaseCostCalibration = None, sampling: BaseSampling = None):
            ...
            assert privacy_unit is not None and isinstance(privacy_unit, TimePrivacyUnit)
    ...
    def cache_calibrate(self, epsilon, delta, cost_name, alphas):
        ...
        # filename = f"{key}_{epsilon}_{delta}_{1.0}.pkl"
        # filepath = f"{mechanism_cache_dir}/{filename}"
        if (epsilon, delta, 1.0) in cache[key]:
            mechanism, info, mcost = cache[key][(epsilon, delta, 1.0)]
        else:
            # if os.path.exists(filepath):
            #     with open(filepath, "rb") as f:
            #         mechanism, info, mcost = pickle.load(f)
            ...
            mcost.set_cost_adp(self.privacy_unit, epsilon, delta)
            mcost.set_cost_rdp(self.privacy_unit, mechanism, alphas)

            cache[key][(epsilon, delta, 1.0)] = (mechanism, info, mcost)

            # with open(filepath, "wb") as f:
            #     pickle.dump((mechanism, info, mcost), f)
    ...
    def poisson_amplify(self, mechanism: AutoDPMechanism, amplify_poisson_lambda: float, cost_name, alphas, epsilon=None, delta=None) -> AutoDPMechanism:
        ...
        if (not skip_cache) and (epsilon, delta, amplify_poisson_lambda) in cache[key]:
            ...
            mcost = MechanismCost(name=cost_name)
            mcost.set_cost_rdp(self.privacy_unit, mechanism_amplified, alphas)
